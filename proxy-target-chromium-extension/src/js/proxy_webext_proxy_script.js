FindProxyForURL = (function () {
  ...
  function FindProxyForURL(url, host, details) {
    ...
    var request = ProxyPac.Conditions.requestFromUrl(url);
    var profile = activeProfile;
    var matchResult, next;
    while (profile) {
      matchResult = ProxyPac.Profiles.match(profile, request)
      if (!matchResult) {
      ...
      if (Array.isArray(matchResult)) {
        ...
        next = ProxyPac.Profiles.nameAsKey(matchResult.profileName)
      } else {
        return fallbackResult;
      }
      profile = ProxyPac.Profiles.byKey(next, options)
    }
    ...
    browser.runtime.onMessage.addListener(function(message) {
      if (message.event === 'proxyScriptStateChanged') {
        ...
        if (!state.currentProfileName) {
          ...
          activeProfile = ProxyPac.Profiles.byName(state.currentProfileName,
            options);
        }
      }
    });
