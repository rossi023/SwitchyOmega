...
Url = require('url')
ContentTypeRejectedError = ProxyTarget.ContentTypeRejectedError
...
xhrWrapper = (url, headers) ->
  ...
  fetch(url, options).then((response) ->
    ...
    if not err.statusCode
      throw new ProxyTarget.NetworkError(err)
    if err.statusCode == 404
      throw new ProxyTarget.HttpNotFoundError(err)
    if err.statusCode >= 500 and err.statusCode < 600
      throw new ProxyTarget.HttpServerError(err)
    throw new ProxyTarget.HttpError(err)
