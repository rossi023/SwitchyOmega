...
class ChromeOptions extends ProxyTarget.Options
  _inspect: null
  _actionForUrl: null
      ...
      .get(TEMPPROFILEKEY).then((tempProfileState = {}) =>
        ...
        if tempProfileState
          ...
          @_tempProfile.rules.forEach((_rule) =>
            key = ProxyPac.Profiles.nameAsKey(_rule.profileName)
  ...
  setMonitorWebRequests: (enabled) ->
    ...
    if enabled and not @_requestMonitor?
      ...
      wildcardForReq = (req) -> ProxyPac.wildcardForUrl(req.url)
  ...
  printFixedProfile: (profile) ->
    ...
    result = ''
    for scheme in ProxyPac.Profiles.schemes when profile[scheme.prop]
      pacResult = ProxyPac.Profiles.pacResult(profile[scheme.prop])
      if scheme.scheme
  ...
  upgrade: (options, changes) ->
    super(options).catch (err) ->
      ...
      return Promise.reject new ProxyTarget.Options.NoOptionsError()
  ...
  getPageInfo: ({tabId, url}) ->
    ...
    Promise.join getBadge, getInspectUrl, (badge, {inspectUrl}) =>
      ...
      domain = ProxyPac.getBaseDomain(Url.parse(url).hostname)
      subdomain = ProxyPac.getSubdomain(url)
