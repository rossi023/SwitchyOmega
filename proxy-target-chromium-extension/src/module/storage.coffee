...
class ChromeStorage extends ProxyTarget.Storage
  @parseStorageErrors: (err) ->
    if err?.message
      ...
      if err.message.indexOf('QUOTA_BYTES_PER_ITEM') >= 0
        err = new ProxyTarget.Storage.QuotaExceededError()
        err.perItem = true
        ...
        err = new ProxyTarget.Storage.RateLimitExceededError()
        ...
        err = new ProxyTarget.Storage.StorageUnavailableError()
