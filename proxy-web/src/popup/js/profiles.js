(function() {
  ...
  function updateMenuByPageInfo() {
    var info = ProxyPopup.pageInfo;
    if (info && info.errorCount > 0) {
      document.querySelector('.om-reqinfo').classList.remove('om-hidden');
      var text = ProxyTargetPopup.getMessage('popup_requestErrorCount',
        [info.errorCount]);
  ...
  function updateMenuByStateAndPageInfo() {
    ...
    ProxyPopup.showTempRuleDropdown = showTempRuleDropdown;
    $script.done('om-dropdowns');
  }

  function showMenuForExternalProfile(state) {
    var profile = state.externalProfile;
    profile.name = ProxyTargetPopup.getMessage('popup_externalProfile')
    ...
    profiles.forEach(function(profile) {
      ...
      link.addEventListener('click', function() {
        $script.ready('om-main', function() {
          ProxyPopup.applyProfile(profile.name);
        });
      });
  ...
  function createMenuItemForProfile(profile, profiles) {
    ...
    var text = ProxyTargetPopup.getMessage('profile_' + profile.name) ||
      profile.name;
  ...
  function createTempRuleDropdown() {
    ...
    var state = ProxyPopup.state;
    var pageInfo = ProxyPopup.pageInfo;
    ...
    profiles.forEach(function(profile) {
      ...
      if ((profile.name === ProxyPopup.state.currentProfileName) &&
      ...
      link.addEventListener('click', function() {
        $script.ready('om-main', function() {
          ProxyPopup.addTempRule(pageInfo.domain, profile.name);
        });
      });
  ...
  function createDefaultProfileDropdown(profile) {
    ...
    profiles.forEach(function(resultProfile) {
      ...
      link.addEventListener('click', function() {
        $script.ready('om-main', function() {
          ProxyPopup.setDefaultProfile(profile.name, resultProfile.name);
        });
      });
