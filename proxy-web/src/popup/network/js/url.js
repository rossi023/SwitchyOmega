...
export const initUrlCellDetail = async (cell) => {
  ...
  const domain = ProxyPac.getBaseDomain(url.hostname);
  const subdomain = ProxyPac.getSubdomain(urlStr);
  ...
  urlContainerEl.querySelector(".add-temp-condition-btn").onclick = () => {
    ...
    tabulatorInstance.clearAlert();
    ProxyTargetPopup.addTempRule(pattern.substring(2), profileName, 1, ()=>{
      ProxyTargetPopup.setState('lastProfileNameForCondition', profileName, ()=>{
        Toastify({
