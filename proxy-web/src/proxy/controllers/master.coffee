angular.module('proxy').controller 'MasterCtrl', ($scope, $rootScope, $window,
  ...
  $rootScope.exportScript = (event, name) ->
    ...
    getProfileName.then (profileName) ->
      ...
      profileNotFound = (name) ->
        missingProfile = name
        return 'dumb'
      ast = ProxyPac.PacGenerator.script($rootScope.options, profileName,
  ...
  $rootScope.profileByName = (name) ->
    ProxyPac.Profiles.byName(name, $rootScope.options)
  ...
  $rootScope.newProfile = ->
    ...
    $modal.open(
      ...
      profile = ProxyPac.Profiles.create(profile)
      ...
      $rootScope.options[ProxyPac.Profiles.nameAsKey(profile)] = profile
      ...
      if profile.profileType == 'FixedProfile'
        ...
        switchProfile = ProxyPac.Profiles.create(
          name: switchName
        ...
        ProxyPac.Profiles.updateRevision(switchProfile)
        $rootScope.options[ProxyPac.Profiles.nameAsKey(switchProfile)] = switchProfile
        attachedName = getAttachedName(switchName)
        attachedKey = ProxyPac.Profiles.nameAsKey(attachedName)
        attached = ProxyPac.Profiles.create(
          name: attachedName
        ...
        ProxyPac.Profiles.updateRevision(attached)
        ...
        if toName != fromName
          ...
          if $rootScope.profileByName(attachedName)
            ...
            if $rootScope.profileByName(toAttachedName)
              ...
              rename = rename.then ->
                toAttachedKey = ProxyPac.Profiles.nameAsKey(toAttachedName)
  ...
  $rootScope.updateProfile = (name) ->
    $rootScope.applyOptionsConfirm().then(->
      if name?
        ...
        ProxyPac.Profiles.each $scope.options, (key, profile) ->
      ...
      proxyTarget.updateProfile(name, 'bypass_cache').then((results) ->
        ...
        if error == 0
          ...
          if error == 1
            singleErr = results[ProxyPac.Profiles.nameAsKey(name)]
            if singleErr
  ...
  for own type of ProxyPac.Profiles.formatByType
  ...
  showFirstRun = ->
    ...
    proxyTarget.state('firstRun').then (firstRun) ->
      ...
      profileName = null
      ProxyPac.Profiles.each $rootScope.options, (key, profile) ->
