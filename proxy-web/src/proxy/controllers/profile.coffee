angular.module('proxy').controller 'ProfileCtrl', ($scope, $stateParams,
  ...
  loadProfile = ->
    ...
    if ProxyPac.Profiles.formatByType[profile.profileType]
      profile.format = ProxyPac.Profiles.formatByType[profile.profileType]
  ...
  $scope.deleteProfile = ->
    ...
    refs = ProxyPac.Profiles.referencedBySet(profileName, $rootScope.options)
    ...
    if Object.keys(refs).length > 0
      ...
      for own key, pname of refs
        ...
        if parent
          key = ProxyPac.Profiles.nameAsKey(parent)
          pname = parent
        refSet[key] = pname

      refProfiles = []
      for own key of refSet
        refProfiles.push(ProxyPac.Profiles.byKey(key, $rootScope.options))
      ...
      $modal.open(
        ...
        delete $rootScope.options[ProxyPac.Profiles.nameAsKey(attachedName)]
        delete $rootScope.options[ProxyPac.Profiles.nameAsKey(profileName)]
