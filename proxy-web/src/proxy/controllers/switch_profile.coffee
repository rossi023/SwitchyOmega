angular.module('proxy').controller 'SwitchProfileCtrl', ($scope, $rootScope,
  ...
  exportRuleList = ->
    text = ProxyPac.RuleList.Switchy.compose(
  ...
  $scope.validateIpCondition = (condition, input) ->
    ...
    ip = ProxyPac.Conditions.parseIp(input)
    return ip?

  $scope.getWeekdayList = ProxyPac.Conditions.getWeekdayList
  ...
  $scope.$watch 'profile.name', (name) ->
    ...
    $scope.attachedKey = ProxyPac.Profiles.nameAsKey($scope.attachedName)
  ...
  $scope.attachNew = ->
    $scope.attached = ProxyPac.Profiles.create(
    ...
    ProxyPac.Profiles.updateRevision($scope.attached)
  ...
  parseProxyRules = (code, {detect, requireResult} = {}) ->
    setError = (error) ->
      ...
      return {error: error}
    if detect and not ProxyPac.RuleList.Switchy.detect(code)
      return {error: {reason: 'notSwitchy'}}
    refs = ProxyPac.RuleList.Switchy.directReferenceSet({
      ruleList: code
    })
    if requireResult and not refs
      return setError({reason: 'resultNotEnabled'})
    for own key, name of refs
      if not ProxyPac.Profiles.byKey(key, $scope.options)
        return setError({reason: 'unknownProfile', args: [name]})
    try
      return rules: ProxyPac.RuleList.Switchy.parseProxy(code, null, null,
  ...
  $scope.toggleSource = -> $q.all([attachedReady, rulesReady]).then ->
    ...
    if $scope.editSource
      args =
        rules: $scope.profile.rules
        defaultProfileName: $scope.attachedOptions.defaultProfileName
      code = ProxyPac.RuleList.Switchy.compose(args, withResult: true)
